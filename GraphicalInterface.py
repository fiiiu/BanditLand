...
class GraphicalInterface():
    ...
    def __init__(self, n_trials, n_blocks):
    ...
    def create_static_screens(self):
        ...
        self.preblockend_screen=self.create_message_screen(u'Calculando juego óptimo...')
        ...
        self.premetacog_screen,self.premetacog2_screen,self.premetacog3_screen=self.create_premetacognitive_screens()

    def create_message_screen(self, text, yoffset=0):
        ...
        message_textpos=message_text.get_rect(centerx=message_screen.get_width()/2, centery=message_screen.get_height()/2+yoffset)
    ...
    def create_premetacognitive_screens(self):
        ...
        premetacog3_screen=self.create_trial_screen(light_background=True)
        font=pygame.font.Font(None, 36)
        premetacog3_screen.blit(premetacog_text, premetacog_textpos)
        premetacog3_text_conf=font.render(" ", 1, (20,20,20))
        premetacog3_textpos_conf=premetacog2_text_conf.get_rect(centerx=premetacog3_screen.get_width()/2, centery=0.7*premetacog3_screen.get_height())
        premetacog3_text_conf_a=font.render(u'alto', 1, (20,20,20))
        premetacog3_text_conf_b=font.render(u'bajo', 1, (20,20,20))
        premetacog3_textpos_conf_a=premetacog3_text_conf_a.get_rect(centerx=self.screen.get_width()*(0.25+parameters.progress_width)+40, \
                                                            centery=0.8*premetacog3_screen.get_height())
        premetacog3_textpos_conf_b=premetacog3_text_conf_b.get_rect(centerx=self.screen.get_width()/4-40, \
                                                            centery=0.8*premetacog3_screen.get_height())
        premetacog3_screen.blit(premetacog3_text_conf, premetacog3_textpos_conf)


        return premetacog_screen, premetacog2_screen, premetacog3_screen
    ...
    def create_metacognitive_screen(self, report_type):
        ...
        if report_type == 0:
            ...
            op=np.random.choice([operator.add, operator.sub])
            opstr="+" if op is operator.add else "-"
            results=np.random.permutation([str(op(a,b)),str(np.random.randint(-20,40))])
            calculation=str(a)+" "+ opstr +" "+str(b)
            metacog_text_conf=font.render(u'¿Cuánto es '+calculation+u'?', 1, (20,20,20))


            metacog_textpos_conf=metacog_text_conf.get_rect(centerx=metacognitive_screen.get_width()/2, centery=0.7*metacognitive_screen.get_height())
            metacog_text_conf_a=font.render(results[0], 1, (20,20,20))
            metacog_text_conf_b=font.render(results[1], 1, (20,20,20))
            metacog_textpos_conf_a=metacog_text_conf_a.get_rect(centerx=self.screen.get_width()*(0.25+parameters.progress_width)+40, \
                                                                centery=0.8*metacognitive_screen.get_height())
            metacog_textpos_conf_b=metacog_text_conf_b.get_rect(centerx=self.screen.get_width()/4-40, \
                                                                centery=0.8*metacognitive_screen.get_height())
            ...
            metacognitive_screen.blit(metacog_text_conf_a, metacog_textpos_conf_a)
            metacognitive_screen.blit(metacog_text_conf_b, metacog_textpos_conf_b)
            result_box_1=pygame.Rect(self.screen.get_width()/4, self.screen.get_height()*0.77, \
                                 self.screen.get_width()*parameters.progress_width/8, \
                                 self.screen.get_height()*parameters.confidence_height)
            result_box_2=pygame.Rect(self.screen.get_width()*5.5/8, self.screen.get_height()*0.77, \
                                 self.screen.get_width()*parameters.progress_width/8, \
                                 self.screen.get_height()*parameters.confidence_height)
            self.box1=pygame.draw.rect(metacognitive_screen, (30,30,30), result_box_1)
            self.box2=pygame.draw.rect(metacognitive_screen, (30,30,30), result_box_2)
            return metacognitive_screen
    ...
    def create_blockend_screen(self, player_reward, optimal_reward, \
                        accumulated_player_reward, accumulated_optimal_reward, block_number):
        ...
        perfo_rect=pygame.Rect(self.screen.get_width()/4, self.screen.get_height()/4, \
                                 self.screen.get_width()*parameters.progress_width, \
                                 parameters.progress_height*1.98)
        ...
        optimal_rect=pygame.Rect(self.screen.get_width()/4, self.screen.get_height()/4+parameters.progress_height,\
        ...
        total_perfo_rect=pygame.Rect(self.screen.get_width()/4, 3*self.screen.get_height()/4, \
                                 self.screen.get_width()*parameters.progress_width, \
                                 parameters.progress_height*2.98)
        ...
        total_optimal_rect=pygame.Rect(self.screen.get_width()/4, 3*self.screen.get_height()/4+parameters.progress_height,\
        ...
        self.total_optimal_perfo=pygame.draw.rect(blockend_screen, (120,0,0), total_optimal_rect)
        self.total_available_perfo=pygame.draw.rect(blockend_screen, (50,0,0), total_available_rect)


        # font=pygame.font.Font(None, 32)
        # player_text=font.render(u'Jugador', 1, (200,200,200))
        # player_textpos=player_text.get_rect(centerx=blockend_screen.get_width()/6, \
        #     centery=self.screen.get_height()/4+10)
        # blockend_screen.blit(player_text, player_textpos)
        # player_text=font.render(u'Óptimo', 1, (200,200,200))
        # player_textpos=player_text.get_rect(centerx=blockend_screen.get_width()/6, \
        #     centery=self.screen.get_height()/4+parameters.progress_height+10)
        # blockend_screen.blit(player_text, player_textpos)
    ...
    def preend_block(self):

        self.screen.blit(self.preblockend_screen, (0,0))
        pygame.display.flip()
    ...
    def end_block(self, player_reward, optimal_reward,\
        ...
        self.blockend_screen=self.create_blockend_screen(player_reward, optimal_reward,\
                             accumulated_player_reward, accumulated_optimal_reward, block_number)
    ...
    def show_metacognitive_screen(self, report_type):
        if report_type==0:
            ...
            pygame.time.delay(200)
            self.metacognitive_screens[1]=self.create_metacognitive_screen(1)
            pygame.draw.rect(self.premetacog3_screen, (30,30,30), self.first_bandit)
            pygame.draw.rect(self.premetacog3_screen, (30,30,30), self.second_bandit)
            self.screen.blit(self.premetacog3_screen, (0,0))
            pygame.display.flip()
            pygame.time.delay(800)
            pygame.draw.rect(self.metacognitive_screens[3], (30,30,30), self.first_bandit)
            pygame.draw.rect(self.metacognitive_screens[3], (30,30,30), self.second_bandit)
            self.screen.blit(self.metacognitive_screens[3], (0,0))
            pygame.display.flip()
            pygame.event.clear()

            waiting_computation=True
            displacement=0
            max_displacement=5
            while waiting_computation:
                events = pygame.event.get()
                for event in events:
                    if event.type == pygame.MOUSEBUTTONUP:
                        if event.button == 1:
                            ...
                            if self.box1.collidepoint(pos):
                                computation=0
                                waiting_computation=False
                            elif self.box2.collidepoint(pos):
                                computation=1
                                waiting_computation=False
            ...
            if computation==0:
                temporary_report=pygame.draw.rect(self.metacognitive_screens[3], (255,255,255), (self.box1.left, self.box1.top, self.box1.width, self.box1.height))
            elif computation==1:
                temporary_report=pygame.draw.rect(self.metacognitive_screens[3], (255,255,255), (self.box2.left, self.box2.top, self.box2.width, self.box2.height))
            self.screen.blit(self.metacognitive_screens[3], (0,0))
            pygame.display.flip()
            pygame.time.delay(200)
    ...
    def end_experiment(self, player_reward, optimal_reward):
        if optimal_reward>0:
            prize=min(30+int(30*float(player_reward)/optimal_reward),60) #linear pay
            #prize=min(30+int(30*(float(player_reward)/optimal_reward)**2),60) #quadratic pay
        else:
            prize=60
        end_screen=self.create_message_screen('Gracias por participar. Usted ha ganado {0} pesos.'.format(prize))
